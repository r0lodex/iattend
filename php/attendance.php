...
function attendanceRead($input) {
	...
	$sql = "SELECT std.fullname,std.matrix_no,DATE_FORMAT(CONVERT_TZ(att.time_reg,'+00:00','+08:00'),'%d %b %Y @ %T') as time_reg,att.id,evn.name FROM attendance att join student std ON std.id=att.student_id join event evn on evn.id=:a WHERE att.event_id=:a";
	$dbc = Database();
	$qry = $dbc->prepare($sql);
	$qry->execute($data);
